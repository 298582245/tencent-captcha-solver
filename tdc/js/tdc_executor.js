...
async function main() {
  ...
  const { tdc_url, ua, trajectory, debug, captcha_origin } = JSON.parse(input);
  ...
  if (debug) log(`fetching tdc.js: ${fullUrl} origin=${env.origin}`);
  const tdcSource = await fetchScript(fullUrl, ua, env.referer, env.origin);
